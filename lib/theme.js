...
import { hexToRgb } from '@utils/javascript';
  ...
  overrides: {
    MuiButton: {
      ...
      containedPrimary: {
        color: 'white',
        boxShadow: () => {
          const { r, g, b } = hexToRgb(tw.theme.colors.primary[600]);

          const color = `${r} ${g} ${b}`;
          return `0px 3px 1px -2px rgb(${color} / 30%), 0px 2px 2px 0px rgb(${color} / 24%), 0px 1px 5px 0px rgb(${color} / 22%)`;
        },
        '&:hover': {
          boxShadow: () => {
            const { r, g, b } = hexToRgb(tw.theme.colors.primary[600]);
            const color = `${r} ${g} ${b}`;
            return `0px 2px 4px -1px rgb(${color} / 40%), 0px 4px 5px 0px rgb(${color} / 34%), 0px 1px 10px 0px rgb(${color} / 32%)`;
          },
        },
      },
    },
